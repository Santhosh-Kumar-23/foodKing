...
const OfferList = props => {
  ...
  const [offer] = useLazyQuery(GET_OFFRS_ID);
  ...
  useFocusEffect(
    useCallback(() => {
      offer({
        ...
        .then(response => {
          console.log('RESSSSSSS', response?.data?.getOfferById.name);
          setOfferList(response?.data?.getOfferById?.addItems ?? []);
          setOthers(response?.data?.getOfferById?.name);
        })
        .catch(error => {
  ...
  return (
    <View style={{flex: 1, backgroundColor: 'white'}}>
      ...
      <View style={{marginHorizontal: 20, backgroundColor: 'white'}}>
        <View style={{height: 100, width: '100%', marginVertical: 8}}>
          <Image
            source={{uri: image}}
            style={{
              height: '100%',
      ...
      {offerList?.length !== 0 ? (
        <View>
          <View
            ...
            <Text style={{color: '#ff146c', fontWeight: '700'}}>{others}</Text>
            <View style={{flexDirection: 'row'}}>
              <TouchableOpacity
                onPress={() => {
                  setChnage(true);
